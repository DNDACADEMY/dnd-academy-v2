...
DND 아카데미 v2 레포를 처음 클론한 뒤 로컬에서 개발 환경을 셋업하는 가이드입니다. Node/pnpm/Lefthook 버전을 [mise](https://mise.jdx.dev/)로 관리합니다.
...
> `mise.toml` 내용을 직접 확인한 뒤 신뢰하세요. 본 레포에는 현재 Node, pnpm, Lefthook 세 가지만 정의돼 있습니다.
...
```bash
pnpm exec lefthook run pre-commit --no-tty
```
...
- **`pnpm install` 마지막에 `lefthook install` 실패 (`core.hooksPath is set locally`)**: 예전 도구가 남긴 로컬 git 설정 때문입니다. 값이 기본 경로(`.git/hooks`)라면 `git config --unset-all --local core.hooksPath` 후 다시 설치하세요.
- **pre-commit이 멈춤**: `pnpm exec lefthook run pre-commit --verbose`로 어떤 명령에서 막혔는지 확인 가능.
