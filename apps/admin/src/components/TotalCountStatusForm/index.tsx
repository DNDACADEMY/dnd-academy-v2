...
function TotalCountStatusForm({ initialTotalCountStatus }: Props) {
  ...
  const { cumulativeApplicants, dropouts, totalParticipants, totalProjects } = initialTotalCountStatus;
  ...
  return (
    <div className={styles.totalCountStatusFormWrapper}>
      <form action={formAction} className={styles.totalCountStatusForm}>
        <input
          type="number"
          name="cumulativeApplicants"
          min={0}
          placeholder="누적 지원자 수"
          defaultValue={cumulativeApplicants}
          required
        />
        <input
          type="number"
      ...
      {state?.message && (
        <div className={clsx(styles.message, state.messageType && styles[state.messageType])}>{state.message}</div>
      )}
    </div>
  );
}
