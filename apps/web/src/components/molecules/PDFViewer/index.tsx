  ...
  return (
    <div ref={containerRef} className={styles.pdfViewerWrapper}>
      ...
      {width && (
        ...
        >
          <Page pageNumber={pageNumber} width={width} loading={<div className={styles.loading}>loading page..</div>} />
        </Document>
      )}
      <div className={styles.pdfButtonWrapper}>
        ...
        <div className={styles.pageCount}>{numPages ? `${pageNumber} / ${numPages}` : pageNumber}</div>
        <Button
        ...
        >
          <ArrowRightIcon
            className={clsx(styles.nextIcon, (!numPages || pageNumber === numPages) && styles.disabled)}
          />
        </Button>
        <Button isExternalLink href={url} buttonType="clear" size="small" theme="light" className={styles.expandLink}>
